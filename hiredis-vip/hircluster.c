...
static int auth_connect(redisClusterContext *cc, redisContext *c)
{
    if (cc->passwd != NULL)
    {
        redisReply *reply;
        ...
        if (reply == NULL){
            __redisClusterSetError(cc, REDIS_ERR_AUTH_FAIL, "auth failed");
            return REDIS_ERR_AUTH_FAIL;            
        }
    ...
    char *str, int str_len, int flags)
{
    ...
    for(pos = start; pos < end; pos ++){
        if(*pos == '\n'){
            ...
            /****************************************************************/
            //去掉redis 4.x版本中添加的redis内部通信端口
            //redis 3.x中的格式是ip:port
            //redis 4.x中的格式是ip:port@InnerPort
            sds redis_addr = part[1];
            int slen = sdslen(redis_addr);
            for (int index = 0; index < slen; index++){
                if (redis_addr[index] == '@') {
                    redis_addr[index] = '\0';
                    sdsupdatelen(redis_addr);
                    break;
                }
            }
            /****************************************************************/
...
int redisClusterSetOptionAuth(redisClusterContext *cc, const char *passwd)
{
...
redisContext *ctx_get_by_node(redisClusterContext *cc, cluster_node *node)
{
    ...
    if (c != NULL && c->err == 0 && auth_connect(cc, c) != REDIS_OK) {
        return NULL;
    }
